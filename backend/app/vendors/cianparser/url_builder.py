...
class URLBuilder:
    ...
    def add_additional_settings(self, additional_settings):
        ...
        if "published_ago" in additional_settings.keys():
            published_value = additional_settings["published_ago"]
            if published_value == "hour":
                self.url += PUBLISHED_AGO_PATH.format(PUBLISHED_AGO_HOUR)
            elif published_value == "today":
                self.url += PUBLISHED_AGO_PATH.format(PUBLISHED_AGO_TODAY)
            elif published_value == "week":
                self.url += PUBLISHED_AGO_PATH.format(PUBLISHED_AGO_WEEK)
            else:
                # Default to today if invalid value
                self.url += PUBLISHED_AGO_PATH.format(PUBLISHED_AGO_TODAY)
        else:
