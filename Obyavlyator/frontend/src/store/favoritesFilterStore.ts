...
import { AdsFilterData } from './adsFilterStore';

interface FavoritesFilterStore {
...
interface FavoritesFilterStoreExtended extends FavoritesFilterStore {
  updateMaxValues: (maxPrice?: number, maxMeters?: number) => void;
  maxValues: { maxPrice: number; maxMeters: number };
}

export const useFavoritesFilter = create<FavoritesFilterStoreExtended>((set, get) => ({
