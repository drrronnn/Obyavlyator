...
class FlatPageParser:
    ...
    def __load_page__(self):
        ...
        time.sleep(4)  # даём странице прогрузиться, можно заменить на WebDriverWait
    ...
    def __parse_flat_offer_page_json__(self):
        ...
        # Парсинг картинок
        pagination_thumbs = self.offer_page_soup.find('ul', {'data-name': 'PaginationThumbsComponent'})
        if pagination_thumbs:
            images = pagination_thumbs.find_all('img')
            for img in images:
                src = img.get('src')
